...
int main(int argc, char *argv[])
{
	...
	if(comm != -1)
	{
		...
		transferBuf = (uint8_t*) malloc(BUFSIZE2);
	        recvBuf = (uint8_t*) malloc(BUFSIZE2);
...
void Tx()
{
	...
	uint8_t * data = (uint8_t*)malloc(8000);
	...
	data_length = fread(data, 1, 8000, tx_fp);
...
int CheckPktTx(uint8_t* data_buf, int data_length, int retryLimit)
{
	...
	transferBuf[BUFSIZE+3] = 0xE5;
	/*push packet check index*/
	transferBuf[BUFSIZE+15] = check_idx_tx;

	if (data_length == 8000){
		/*copy data length*/
		memcpy(transferBuf + 16, &data_full, sizeof(int));
		
		/*copy data*/
		memcpy(transferBuf + 20, data_buf, 4000);

		/*copy data length*/
		memcpy(transferBuf + 16 + BUFSIZE, &data_full, sizeof(int));
		
		/*copy data*/
		memcpy(transferBuf + 20 + BUFSIZE, data_buf+4000, 4000);
	} else if (data_length > 4000){
		/*copy data length*/
		memcpy(transferBuf + 16, &data_full, sizeof(int));
		...
		/*copy data length*/
		data_length = data_length - 4000;
		memcpy(transferBuf + 16 + BUFSIZE, &data_length, sizeof(int));
		
		/*copy data*/
		memcpy(transferBuf + 20 + BUFSIZE, data_buf+4000, data_length);
	} else {
		/*copy data length*/
	...
	for(int i = 0; i < 4; i++)
	{
		...
		transferBuf[BUFSIZE+4092+i] = 0x00;
	}
		...
		status = ML_Transfer(transferBuf, pkt_length);
			...
			status = ML_Receiver(recvBuf, &length);
		        if(status > 0)
				{
                	...
                	if(recvBuf[15] == check_idx_tx && recvBuf[BUFSIZE+15] == check_idx_tx)
					{
			            check_idx_tx++;
			            tx_cnt++;

			            std::cout << "\rTransfer progress: " <<  std::setw(3) << ((tx_cnt*800000)/fileSize) << "% \t";
			            end = std::chrono::steady_clock::now();
			            // alpha*old_speed + (1.0f-alpha)*
						speed = alpha*old_speed + (1.0f-alpha)*64000.0f/std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count();
...
int CheckPktRx(uint8_t* data_buf,int retryLimit)
{
	...
	for(int i = 0; i< retryLimit; i++)
	{
		...
		status = ML_Receiver(recvBuf, &length);
	        if(status > 0)
        	{
			if((check_idx_rx != recvBuf[15] && recvBuf[3] == 0xE5) || (recvBuf[15] == recvBuf[BUFSIZE+15] && recvBuf[BUFSIZE+3] == 0xE5))
		        {
				...
				if(data_length > 4000 || data_length <0)
					return 0;
		        		...
		        		memcpy(&data_length, recvBuf+16+BUFSIZE, sizeof(int));
		        		total_length = total_length + data_length;
		        		memcpy(data_buf+4000, recvBuf+20+BUFSIZE, data_length);
